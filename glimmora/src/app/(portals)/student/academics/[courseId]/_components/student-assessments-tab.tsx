...
import { useStudentAssessments } from "@/lib/hooks/use-student";
import { useStartAssessmentAttempt } from "@/lib/hooks/use-student";
...
export function StudentAssessmentsTab({ courseId }: { courseId: string }) {
  ...
  const { data, isLoading, isError, refetch } = useStudentAssessments(courseId);
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        {list.map((a) => (
          <AssessmentCard
            ...
            onResume={() =>
              router.push(`/student/academics/${courseId}/assessments/${a.id}/take`)
            }
            onViewResult={() => {
        ...
        return {
          ...
          cta: (
            ...
            >
              ...
              View result
              {a.attemptsUsed < a.attemptsAllowed && a.studentStatus === "completed" && (
