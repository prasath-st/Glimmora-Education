...
function UserMenu() {
  ...
  const handleLogout = async () => { setOpen(false); await logout(); router.push("/login"); };
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-60 overflow-hidden rounded-lg bg-card shadow-2xl ring-1 ring-border/50">
          ...
          <div className="border-t border-border/50 py-1">
            <Link href="/super-admin/settings?tab=profile" onClick={() => setOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm transition-colors hover:bg-muted">
              <UserIcon className="h-4 w-4 text-muted-foreground" /> Profile
            </Link>
            <Link href="/super-admin/settings?tab=platform" onClick={() => setOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm transition-colors hover:bg-muted">
